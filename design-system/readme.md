...
- **Icon sprite.** `assets/icons/void-icons.svg` is present: 44 Lucide symbols (ISC), self-hosted,
  read through `<use>` by the `Icon` component. Symbols carry geometry only; `Icon` supplies
  stroke, width, linecap and linejoin so one sprite serves every size and colour. Inventory and
  call sites in **`assets/icons/README.md`**.
...
**Root**
- `tokens.json` — **the contract.** SRS Appendix I in full: two tiers, both schemes, every
  semantic token with its `cssVar`, plus `meta.sourceProvenance`, `meta.adaptations` and the
  V1–V12 status. `tokens/*.css` is the emitted artefact; this file is the identity.
...
**`docs/`**
- `adr/0007-design-source.md` — the intake, the trade-dress position, every adaptation made for
  an invariant, and every decision made in the Product Owner's absence. Read before changing a value.
- `adr/0001-theme-payload-budget.md` — the 40KB critical-path budget, as supplied.
- `licences/README.md` — what ships, under what licence, with one outstanding drop (Geist's OFL text).
- `HANDOFF.md` — the implementation guide: what transfers literally, the non-negotiable rules, the
  five CI gates still to build, and the requirement coverage map.
...
**`templates/storefront/`** — built. Eleven screens: homepage, listing with filters, product
detail, bag, checkout, confirmation, order detail, account, help centre, and the branded 404 and
500. Each at 1440 and 320, in light and dark, in two locales, in four states (populated, loading
skeleton, empty, error with retry). This is the artefact UI-SRC-8 asks for: one harness driven by
props rather than a separate storybook. `templates/storefront/README.md` documents the axes and
the state rules.

It lives under `templates/` rather than `ui_kits/` because `@startingPoint` is no longer honoured
by consuming projects; a `templates/<slug>/` folder is. `Storefront.dc.html` is the entry, and
`ds-base.js` is the single line a consuming project repoints at its own `_ds/` tree.

*Remaining substitution:* photography. Product plates render as grey blocks at their real aspect
ratios (4:5 portrait, 1:1 thumbnails) so composition reads correctly without standing in for art
direction. Decided 2026-09-05 — placeholders stay until real shots land in `assets/photography/`.

**`templates/admin-dashboard/`** — built. The back-office shell (sidebar, topbar, global search,
market switcher, colour-scheme toggle) plus two finished pages: an overview dashboard and the dense
order list. `AdminDashboard.dc.html` is the entry. All eleven nav destinations are built: overview, orders,
customs queue, moderation, returns, payouts, catalogue, vendors, designers, markets, the design
token editor (UI-SRC-14) and the back-office help.

Structure follows the AdminCN reference the Product Owner supplied on 2026-09-05 — persistent
sidebar with uppercase tracked group labels, slim topbar with breadcrumb, thin-bordered surfaces
with an icon chip on each statistic, nested metric tiles, a donut for goal progress. The reference
supplied the *layout vocabulary only*; every colour, size and radius resolves through a Void token
(UI-SRC-9), and the accent appears exactly where the reference used its primary tint. Requirements
covered: UI-ADM-1 (density, bulk selection, saved views), UI-ADM-2 (one global input), UI-ADM-3
(column configuration, filtering, CSV export of the current view), UI-ADM-4 (confirmation naming
the exact effect and value), UI-ADM-5 (market switcher plus a per-screen scope line), FR-ADM-52
(customs queue with age and the carrier's stated reason), UI-PTR-1 (density derived from the
storefront token set, not a second theme).

## Surface coverage against the SRS

| Surface | Requirements | State |
| --- | --- | --- |
| Product listing | UI-PLP-1..7 | Built — `templates/storefront/` |
| Product detail | UI-PDP-1..9 | Built — `templates/storefront/` |
| Checkout step | UI-CHK-1..9 | Built — one step of the flow |
| Account hub | UI-ACC-1 | Built — `templates/storefront/` |
| Admin shell, overview, orders | UI-ADM-1..5, FR-ADM-52 | Built — `templates/admin-dashboard/` |
| Homepage | UI-HOME-1..7 | Built — `templates/storefront/` |
| Cart | UI-CHK-1..6 | Built — `templates/storefront/` |
| Order confirmation | UI-CHK-9, FR-AUTH-7 | Built — `templates/storefront/` |
| Order detail with customs timeline | UI-ACC-2, UI-ACC-3 | Built — `templates/storefront/` |
| Void Studio | UI-STU-1..10 | Built — `templates/studio/` |
| Authentication set | FR-AUTH-1..19 | Built — `templates/auth/`, 10 screens |
| Vendor portal | UI-PTR-1..4 | Built — `templates/portals/` |
| Designer portal | UI-PTR-1..3 | Built — `templates/portals/` |
| Admin operational destinations | FR-ADM-*, UI-SRC-14 | Built — `templates/admin-dashboard/`, 11 destinations |
| Help centre | UI-ACC-4 | Built — `templates/storefront/`, `help` screen |
| Branded 404 and 500 | UI-GLOB-6 | Built — `templates/storefront/`, `notfound` / `servererror` |
| Token contract | Appendix I, V1–V12 | Built — `tokens.json` |
| Design source record | UI-SRC-3, 5, 12 | Built — `docs/adr/0007-design-source.md` |
| Licence record | V11 | Built — `docs/licences/`; Geist OFL text outstanding |
| Both locales on the preview surfaces | UI-SRC-8 | **Partial** — type stack, leading and numerals correct under `lang="bn"`; copy untranslated |
| Placeholder token set | UI-SRC-7 | **Not built** — deliberately; see `docs/HANDOFF.md` |
| Lint rule, validator, theme-swap test, budget checks | UI-SRC-9..11, ADR-0001 | **Not built** — engineering; specified in `docs/HANDOFF.md` |

**`templates/studio/`** — built. The full-viewport garment design surface: tool rail, neutral
canvas, properties panel with a live price breakdown, print warnings attached to the offending
element and gathered again as a pre-order summary, first-run walkthrough, and a mobile layout that
swaps the side panels for a bottom toolbar and sheets. UI-STU-10 is the one place the design system
is deliberately overridden — the canvas surround is a fixed neutral grey that does not follow the
colour scheme, because a customer judges artwork colour against it.

**`templates/auth/`** — built. Ten screens: sign in (clean and failed), registration with a
strength meter that reports length and breach status rather than composition rules, SMS and
authenticator codes with validity and attempt budgets on screen, password reset and its
deliberately non-committal sent state, the self-lifting 15-minute lockout, guest order lookup, and
the active-session list.

**`templates/portals/`** — built. Both partner surfaces from one shell, switched by a `role` prop:
the vendor's order queue (accept and dispatch), catalogue, per-product market coverage with the
blocking reason named per market, and settlement; the designer's designs, performance by market and
royalties. UI-PTR-2 shaped the layout more than anything else — at 320 the sidebar becomes a bottom
bar and each order becomes a card whose primary action is a full-width button, because a vendor
accepts orders from a phone in a warehouse. Every figure states its period, its currency and
whether it is pending or available (UI-PTR-3); there is no bare number on either surface.

Still open: Bengali interface copy, real photography, Geist's OFL text, and the five CI gates
specified in `docs/HANDOFF.md`. Nothing in Chapter 9 is undesigned.
